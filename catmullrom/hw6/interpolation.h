...
namespace Interpolation {
	...
	inline Cvec3 Bezier(Cvec3 c0, Cvec3 d0, Cvec3 e0, Cvec3 c1, const double& alpha) {
		const Cvec3 f = c0 * (1 - alpha) + d0 * alpha;
		const Cvec3 g = d0 * (1 - alpha) + e0 * alpha;
		const Cvec3 h = e0 * (1 - alpha) + c1 * alpha;
		const Cvec3 m = f * (1 - alpha) + g * alpha;
		const Cvec3 n = g * (1 - alpha) + h * alpha;
		return m * (1 - alpha) + n * alpha;
	}
	...
	inline RigTForm CatmullRom(const RigTForm& rbt_i, const RigTForm& rbt0, const RigTForm& rbt1, const RigTForm& rbt_f, const double& alpha) {
		...
		const Quat d0_quat = pow(q1 * inv(rbt_i.getRotation()), 1 / 6) * q0;
		const Quat e0_quat = pow(rbt_f.getRotation() * inv(q0), -1 / 6) * q1;
