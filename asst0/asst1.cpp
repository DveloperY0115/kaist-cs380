...
static void keyboard(unsigned char key, int x, int y){
  /* for asst0 */
    switch (key) {
        // For input 'h', say hello
    case 'h':
        cout << "Welcome to OpenGL! \n";
        cout << "Please do enjoy studying Ahahaha! \n";
        break;
        // For input 'q', quit
    case 'q':
        cout << "Got input quit. Terminating.. \n";
        exit(0);
...
static void mouse(int button, int state, int x, int y) { 
  /* for asst0 */
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            // Left mouse clicked -> Move triangle to the right
        ...
        if (state == GLUT_DOWN)
            // Right mouse clicked -> Move triangle to the left
